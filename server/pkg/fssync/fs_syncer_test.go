...
func TestNewFsSyncer_Success(t *testing.T) {
	...
	mockDB.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(&model.Directory{Path: testDir}, nil)
	...
	mockDB.EXPECT().GetAllDirectories(gomock.Any()).Return(&[]model.Directory{
	...
	mockDIG.EXPECT().GetBelongingItems(gomock.Any(), gomock.Any()).Return(&[]model.Item{}, nil)

	syncer, err := newFsSyncer(ctx, testDir, mockDB, mockDIG, trueFileFilter{})
...
func TestNewFsSyncer_DirectoryNotFound(t *testing.T) {
	...
	mockDB.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)

	mockDIG := model.NewMockDirectoryItemsGetter(ctrl)

	syncer, err := newFsSyncer(ctx, "/nonexistent", mockDB, mockDIG, trueFileFilter{})
...
func TestNewFsSyncer_DirectoryCheckError(t *testing.T) {
	...
	// Mock database error
	mockDB.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, errors.New("database error"))

	mockDIG := model.NewMockDirectoryItemsGetter(ctrl)

	syncer, err := newFsSyncer(ctx, "/test", mockDB, mockDIG, trueFileFilter{})
...
func TestSync_NoChanges(t *testing.T) {
	...
	mockDB.EXPECT().GetAllDirectories(gomock.Any()).Return(&[]model.Directory{}, nil)

	// Mock for syncAutoTags - should get all directories again
	mockDB.EXPECT().GetAllDirectories(gomock.Any()).Return(&[]model.Directory{}, nil)

	hasChanges, errs := syncer.sync(ctx, mockDB, mockDIGS, mockDATG, mockFMG)
...
func TestSync_WithChanges(t *testing.T) {
	...
	mockDIGS.EXPECT().GetBelongingItem(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil, nil).AnyTimes()
	mockDB.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound).AnyTimes()
	mockDB.EXPECT().RemoveDirectory(gomock.Any(), gomock.Any()).Return(nil).AnyTimes()

	// Mocks for adding directories
	mockDB.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound).AnyTimes()
	mockDB.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Return(nil).AnyTimes()

	// Mock AutoTags for new files
	mockDATG.EXPECT().GetAutoTags(gomock.Any(), gomock.Any()).Return([]*model.Tag{}, nil).AnyTimes()
	mockFMG.EXPECT().GetFileMetadata(gomock.Any()).Return(int64(12345), int64(1000), nil).AnyTimes()
	mockDIGS.EXPECT().AddBelongingItem(gomock.Any(), gomock.Any()).Return(nil).AnyTimes()
...
func TestAddMissingDirectoryTags_Success(t *testing.T) {
	...
	mockDR.EXPECT().GetAllDirectories(gomock.Any()).Return(&testDirectories, nil)

	// Mock the tag operations for each directory
	for range testDirectories {
		mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
		mockTRW.EXPECT().CreateOrUpdateTag(gomock.Any(), gomock.Any()).Return(nil)
	}

	errs := addMissingDirectoryTags(ctx, mockDR, mockTRW)
...
func TestAddMissingDirectoryTags_GetDirectoriesError(t *testing.T) {
	...
	mockDR.EXPECT().GetAllDirectories(gomock.Any()).Return(nil, errors.New("db error"))
...
func TestAddMissingDirectoryTag_ExcludedDirectory(t *testing.T) {
	...
	err := addMissingDirectoryTag(ctx, mockTRW, excludedDir)
...
func TestAddMissingDirectoryTag_Success(t *testing.T) {
	...
	err := addMissingDirectoryTag(ctx, mockTRW, dir)
...
func TestRemoveStaleItems_Success(t *testing.T) {
	...
	for range files {
		mockDIG.EXPECT().GetBelongingItem(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil, nil)
	}

	errs := removeStaleItems(ctx, mockDIG, mockIW, files)
...
func TestRemoveStaleItems_WithExistingItems(t *testing.T) {
	...
	mockDIG.EXPECT().GetBelongingItem(gomock.Any(), gomock.Any(), gomock.Any()).Return(testItem, nil)
	mockIW.EXPECT().RemoveItem(gomock.Any(), testItem.Id).Return(nil)
...
func TestRemoveStaleDirs_Success(t *testing.T) {
	...
	for range dirs {
		...
		mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
		mockDW.EXPECT().RemoveDirectory(gomock.Any(), gomock.Any()).Return(nil)
	}

	errs := removeStaleDirs(ctx, mockTRW, mockDW, dirs)
...
func TestAddMissingDirs_Success(t *testing.T) {
	...
	for range changes {
		...
		mockDRW.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
		mockDRW.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Return(nil)
	}

	errs := addMissingDirs(ctx, mockDRW, changes)
...
func TestAddNewFiles_Success(t *testing.T) {
	...
	mockDIGS.EXPECT().GetBelongingItem(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil, nil)

	// Mock AutoTags for the directory
	mockDATG.EXPECT().GetAutoTags(gomock.Any(), gomock.Any()).Return([]*model.Tag{}, nil)

	// Mock file metadata
	mockFMG.EXPECT().GetFileMetadata(gomock.Any()).Return(int64(12345), int64(1000), nil)

	// Mock adding the new item
	mockDIGS.EXPECT().AddBelongingItem(gomock.Any(), gomock.Any()).Return(nil)

	errs := addNewFiles(ctx, mockIW, mockDIGS, mockDATG, mockFMG, changes)
...
func TestAddNewFiles_ExistingItem(t *testing.T) {
	...
	mockDIGS.EXPECT().GetBelongingItem(gomock.Any(), gomock.Any(), gomock.Any()).Return(existingItem, nil)
...
func TestRenameDirs_Success(t *testing.T) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(dstDir, nil)

	// Mock source directory exists
	srcDir := createTestDir("/test/old-dir", false)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(srcDir, nil)

	// Mock updating directory path
	mockDRW.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Return(nil)
	mockDRW.EXPECT().RemoveDirectory(gomock.Any(), gomock.Any()).Return(nil)
	...
	mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(dirTag, nil)
	// Note: GetItems is not called when items list is empty because dbBelongingItemDbReader.GetItems returns early

	errs := renameDirs(ctx, mockTRW, mockDRW, mockIRW, changes)
...
func TestRenameFiles_Success(t *testing.T) {
	...
	mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(srcTag, nil)
	// Mock items in source directory through GetItems call
	mockIRW.EXPECT().GetItems(gomock.Any(), gomock.Any()).Return(&[]model.Item{*originalItem}, nil)

	// Mock updating item location
	mockIRW.EXPECT().UpdateItem(gomock.Any(), gomock.Any()).Return(nil)

	// Mock destination directory tag for adding item
	mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(srcTag, nil) // Same tag since same directory
	mockIRW.EXPECT().CreateOrUpdateItem(gomock.Any(), gomock.Any()).Return(nil)

	// Mock source directory tag for removing item
	mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(srcTag, nil)
	mockIRW.EXPECT().RemoveTagFromItem(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)

	errs := renameFiles(ctx, mockTRW, mockDRW, mockIRW, changes)
...
func TestSyncAutoTags_Success(t *testing.T) {
	...
	for i, dir := range testDirectories {
		...
		mockDATG.EXPECT().GetAutoTags(gomock.Any(), dir.Path).Return([]*model.Tag{}, nil)
		...
		mockTR.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(tag, nil)
		// Note: GetItems is not called when items list is empty because dbBelongingItemDbReader.GetItems returns early
	}

	_, errs := syncAutoTags(ctx, mockTR, mockIRW, mockDR, mockDATG)
...
func TestSyncAutoTagsForDir_Success(t *testing.T) {
	...
	mockTR.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(dirTag, nil)

	// Mock items that need the auto tags
	items := []model.Item{*testItem}
	mockIRW.EXPECT().GetItems(gomock.Any(), gomock.Any()).Return(&items, nil)

	// Mock updating item with auto tags
	mockIRW.EXPECT().UpdateItem(gomock.Any(), gomock.Any()).Return(nil)

	_, errs := syncAutoTagsForDir(ctx, mockTR, mockIRW, autoTags, dir)
...
func TestSync_ErrorsCollected(t *testing.T) {
	...
	mockDB.EXPECT().GetAllDirectories(gomock.Any()).Return(nil, errors.New("first error"))

	// Force an error in syncAutoTags
	mockDB.EXPECT().GetAllDirectories(gomock.Any()).Return(nil, errors.New("second error"))
...
func TestRemoveDir_TagNotFound(t *testing.T) {
	...
	errs := removeDir(ctx, mockTRW, mockDW, "/test/path")
...
func TestRemoveDir_TagFoundAndRemoved(t *testing.T) {
	...
	// Mock tag found
	mockTRW.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(tag, nil)

	// Mock tag removal
	mockTRW.EXPECT().RemoveTag(gomock.Any(), tag.Id).Return(nil)
...
func TestMoveDir_DestinationNotExists(t *testing.T) {
	...
	errs := moveDir(ctx, mockTRW, mockDRW, mockIRW, "/src", "/dst")
...
func TestMoveFile_ItemNotFound(t *testing.T) {
	...
	err := moveFile(ctx, mockTRW, mockDRW, mockIRW, "/test/src.mp4", "/test/dst.mp4")
...
func TestAddMissingDirectoryTags2(t *testing.T) {
	...
	dr.EXPECT().GetAllDirectories(gomock.Any()).Return(&[]model.Directory{
		{Path: "dir1"},
		{Path: "dir-2"},
		{Path: "dir_3"},
	...
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, nil)
	trw.EXPECT().CreateOrUpdateTag(gomock.Any(), &model.Tag{Title: "dir1", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())}).Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, nil)
	trw.EXPECT().CreateOrUpdateTag(gomock.Any(), &model.Tag{Title: "dir-2", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())}).Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, nil)
	trw.EXPECT().CreateOrUpdateTag(gomock.Any(), &model.Tag{Title: "dir_3", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())}).Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, nil)
	trw.EXPECT().CreateOrUpdateTag(gomock.Any(), &model.Tag{Title: "some/inner/path", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())}).Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(&model.Tag{Title: "tag/exists", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())}, nil)
	errs := addMissingDirectoryTags(ctx, dr, trw)
...
func TestAddMissingDirs2(t *testing.T) {
	...
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().CreateOrUpdateDirectory(gomock.Any(), &model.Directory{Path: "path1", Excluded: pointer.Bool(true)}).Return(nil)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().CreateOrUpdateDirectory(gomock.Any(), &model.Directory{Path: "some/path - to dir", Excluded: pointer.Bool(true)}).Return(nil)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(&model.Directory{Path: "/absolute/exists/path3"}, nil)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().CreateOrUpdateDirectory(gomock.Any(), &model.Directory{Path: "/absolute/error/inner/path4", Excluded: pointer.Bool(true)}).Return(errors.Errorf("test"))
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().CreateOrUpdateDirectory(gomock.Any(), &model.Directory{Path: model.ROOT_DIRECTORY_PATH, Excluded: pointer.Bool(true)}).Return(nil)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(&model.Directory{Path: model.ROOT_DIRECTORY_PATH}, nil)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().CreateOrUpdateDirectory(gomock.Any(), &model.Directory{Path: "some/relative/path", Excluded: pointer.Bool(true)}).Return(nil)

	errs := addMissingDirs(ctx, drw, []directorytree.Change{
		{Path1: "path1"},
...
func TestAddMissingDirs_AutoIncludeChildren_True(t *testing.T) {
	...
	for range changes {
		...
		mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "parent").Return(parentDir, nil)

		// AddDirectoryIfMissing -> DirectoryExists -> GetDirectory
		mockDRW.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)

		// CreateOrUpdateDirectory with excluded=false (shouldInclude=true)
		mockDRW.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, dir *model.Directory) {
...
func TestAddMissingDirs_AutoIncludeChildren_False(t *testing.T) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", model.ROOT_DIRECTORY_PATH).Return(nil, gorm.ErrRecordNotFound)

	// AddDirectoryIfMissing -> DirectoryExists -> GetDirectory
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "parent/child").Return(nil, gorm.ErrRecordNotFound)
...
func TestAddMissingDirs_AutoIncludeHierarchy_True(t *testing.T) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "grandparent/parent").Return(parentDir, nil)

	// Since parent doesn't have AutoIncludeChildren=true, check hierarchy
	// GetParent of "grandparent/parent" -> returns "grandparent"
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "grandparent").Return(grandparentDir, nil)
...
func TestAddMissingDirs_AutoIncludeHierarchy_DeepNesting(t *testing.T) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "root/level1/level2/level3").Return(level3Dir, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "root/level1/level2").Return(level2Dir, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "root/level1").Return(level1Dir, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "root").Return(rootDir, nil)
...
func TestAddMissingDirs_ParentNotFound(t *testing.T) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "nonexistent").Return(nil, gorm.ErrRecordNotFound)

	// AddDirectoryIfMissing -> DirectoryExists -> GetDirectory
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "nonexistent/child").Return(nil, gorm.ErrRecordNotFound)
...
func TestAddMissingDirs_MixedAutoIncludeFlags(t *testing.T) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "include-children").Return(parentWithChildren, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "include-children/child1").Return(nil, gorm.ErrRecordNotFound)
	mockDRW.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, dir *model.Directory) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "include-hierarchy/level1").Return(level1Dir, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "include-hierarchy").Return(parentWithHierarchy, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "include-hierarchy/level1/level2").Return(nil, gorm.ErrRecordNotFound)
	mockDRW.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, dir *model.Directory) {
	...
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "no-flags").Return(parentWithoutFlags, nil)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", model.ROOT_DIRECTORY_PATH).Return(nil, gorm.ErrRecordNotFound)
	mockDRW.EXPECT().GetDirectory(gomock.Any(), "path = ?", "no-flags/child").Return(nil, gorm.ErrRecordNotFound)
	mockDRW.EXPECT().CreateOrUpdateDirectory(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, dir *model.Directory) {
...
func TestAddNewFiles2(t *testing.T) {
	...
	digs.EXPECT().GetBelongingItem(gomock.Any(), "new", "file").Return(&model.Item{Title: "file", Origin: "new", Url: "new/file", LastModified: 4234234, Tags: tags}, nil)
	autoTags1 := []*model.Tag{{Id: 1, Title: "auto1"}, {Id: 2, Title: "auto2"}}
	datg.EXPECT().GetAutoTags(gomock.Any(), "new").Return(autoTags1, nil)
	iw.EXPECT().UpdateItem(gomock.Any(), &model.Item{Title: "file", Origin: "new", Url: "new/file", LastModified: 4234234, Tags: append(tags, autoTags1...)})

	digs.EXPECT().GetBelongingItem(gomock.Any(), "some/deep/deep/deep", "file").Return(nil, nil)
	autoTags2 := []*model.Tag{{Title: "auto1"}, {Title: "auto2"}}
	datg.EXPECT().GetAutoTags(gomock.Any(), "some/deep/deep/deep").Return(autoTags2, nil)
	fmg.EXPECT().GetFileMetadata("/root/dir/some/deep/deep/deep/file").Return(int64(7657657), int64(0), nil)
	digs.EXPECT().AddBelongingItem(gomock.Any(), &model.Item{Title: "file", Origin: "some/deep/deep/deep", Url: "some/deep/deep/deep/file", LastModified: 7657657, Tags: autoTags2}).Return(nil)

	digs.EXPECT().GetBelongingItem(gomock.Any(), "/absolute", "path").Return(nil, nil)
	datg.EXPECT().GetAutoTags(gomock.Any(), "/absolute").Return([]*model.Tag{}, nil)
	fmg.EXPECT().GetFileMetadata("/absolute/path").Return(int64(7567657), int64(0), nil)
	digs.EXPECT().AddBelongingItem(gomock.Any(), &model.Item{Title: "path", Origin: "/absolute", Url: "/absolute/path", LastModified: 7567657, Tags: make([]*model.Tag, 0)}).Return(nil)

	digs.EXPECT().GetBelongingItem(gomock.Any(), "some", "file").Return(nil, nil)
	datg.EXPECT().GetAutoTags(gomock.Any(), "some").Return([]*model.Tag{}, nil)
	fmg.EXPECT().GetFileMetadata("/root/dir/some/file").Return(int64(9876532), int64(0), nil)
	digs.EXPECT().AddBelongingItem(gomock.Any(), &model.Item{Title: "file", Origin: "some", Url: "some/file", LastModified: 9876532, Tags: make([]*model.Tag, 0)}).Return(nil)

	errs := addNewFiles(ctx, iw, digs, datg, fmg, []directorytree.Change{
		{Path1: "new/file"},
...
func TestRemoveStaleDirs2(t *testing.T) {
	...
	relativasor.Init("/root/dir")
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(&model.Tag{Id: 1, Title: "tag1"}, nil)
	trw.EXPECT().RemoveTag(gomock.Any(), uint64(1)).Return(nil)
	dw.EXPECT().RemoveDirectory(gomock.Any(), "/existing/dir").Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, nil)
	dw.EXPECT().RemoveDirectory(gomock.Any(), "/not/exists/tag").Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(&model.Tag{Id: 2, Title: "relative"}, nil)
	trw.EXPECT().RemoveTag(gomock.Any(), uint64(2)).Return(nil)
	dw.EXPECT().RemoveDirectory(gomock.Any(), "relative/path").Return(nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(&model.Tag{Id: 3, Title: model.ROOT_DIRECTORY_PATH}, nil)
	trw.EXPECT().RemoveTag(gomock.Any(), uint64(3)).Return(nil)
	dw.EXPECT().RemoveDirectory(gomock.Any(), model.ROOT_DIRECTORY_PATH).Return(nil)

	errs := removeStaleDirs(ctx, trw, dw, []string{
		"/existing/dir",
		"/not/exists/tag",
...
func TestRemoveStaleItems2(t *testing.T) {
	...
	relativasor.Init("/root/dir")
	dig.EXPECT().GetBelongingItem(gomock.Any(), "/some", "not-exists").Return(nil, nil)
	dig.EXPECT().GetBelongingItem(gomock.Any(), "/some/inner", "file").Return(&model.Item{Id: 1}, nil)
	iw.EXPECT().RemoveItem(gomock.Any(), uint64(1)).Return(nil)
	dig.EXPECT().GetBelongingItem(gomock.Any(), "relative", "file").Return(&model.Item{Id: 2}, nil)
	iw.EXPECT().RemoveItem(gomock.Any(), uint64(2)).Return(nil)

	errs := removeStaleItems(ctx, dig, iw, []string{
		"/some/not-exists",
		"/some/inner/file",
...
func TestRenameFiles2(t *testing.T) {
	...
	relativasor.Init("/root/dir")
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	drw.EXPECT().CreateOrUpdateDirectory(gomock.Any(), &model.Directory{Path: "/new", Excluded: pointer.Bool(false)}).Return(nil)
	drw.EXPECT().GetDirectory(gomock.Any(), gomock.Any()).Return(&model.Directory{Path: "/new", Excluded: pointer.Bool(false)}, nil)
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
	trw.EXPECT().CreateOrUpdateTag(gomock.Any(), &model.Tag{Title: "/new", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())})
	oldTag := &model.Tag{Id: 4321, Title: "/", ParentID: pointer.Uint64(directories.GetDirectoriesTagId()), Items: []*model.Item{{Id: 1}}}
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(oldTag, nil)
	irw.EXPECT().GetItems(gomock.Any(), gomock.Any()).Return(&[]model.Item{{Id: 1, Title: "file1"}}, nil)
	irw.EXPECT().UpdateItem(gomock.Any(), &model.Item{Id: 1, Title: "file1", Origin: "/new", Url: "/new/file1"}).Return(nil)
	newTag := &model.Tag{Title: "new", ParentID: pointer.Uint64(directories.GetDirectoriesTagId())}
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(newTag, nil)
	irw.EXPECT().CreateOrUpdateItem(gomock.Any(), &model.Item{Id: 1, Title: "file1", Origin: "/new", Url: "/new/file1", Tags: []*model.Tag{newTag}})
	trw.EXPECT().GetTag(gomock.Any(), gomock.Any()).Return(oldTag, nil)
	irw.EXPECT().RemoveTagFromItem(gomock.Any(), uint64(1), uint64(4321)).Return(nil)

	errs := renameFiles(ctx, trw, drw, irw, []directorytree.Change{
